...
const PROJECT_SLUG = "ikea-embedded-storage";
  ...
  switch (block.type) {
    case "header": {
      ...
      return (
        <HeaderBlock
          ...
          heroVideoScale={block.heroVideoScale}
    ...
    default:
      if (process.env.NODE_ENV === "development") {
        console.warn("[IkeaEmbeddedStoragePage] Unknown block type:", (block as { type: string }).type);
      }
      return null;
  }
}

export default function IkeaEmbeddedStoragePage() {
  ...
  useEffect(() => {
    fetch(`${MEDIA_BASE}/manifest.json`, { cache: "no-store" })
      .then((res) => {
        if (!res.ok) {
