...
import { loopCopy, loopOverview, type LoopContentKey } from "@/content/projects/loop";
...
type ManifestBlock =
  ...
  | { type: "text"; heading?: string; content?: string; contentKey?: LoopContentKey }
  | { type: "quote"; content?: string; contentKey?: LoopContentKey; emphasis?: string; size?: "default" | "large" }
  | {
      type: "stats";
...
function renderBlock(
  ...
  switch (block.type) {
    ...
    case "text": {
      ...
      let content: string;
      if (block.contentKey) {
        const section = loopCopy[block.contentKey];
    ...
    case "quote": {
      ...
      if (block.contentKey) {
        const raw = loopCopy[block.contentKey];
    ...
    default:
      if (process.env.NODE_ENV === "development") {
        console.warn("[LoopPage] Unknown block type:", (block as any).type);
      }
      return null;
  }
}
...
export default function LoopPage() {
  ...
  const project = getProjectBySlug("loop");
  ...
  useEffect(() => {
    fetch("/media/projects/loop/manifest.json", { cache: "no-store" })
      .then((res) => {
        if (!res.ok) {
  ...
  return (
    <ProjectMediaBaseProvider basePath="/media/projects/loop">
      <main className="pt-[144px] pb-0 relative">
        <Container>
          <div ref={rootRef}>
