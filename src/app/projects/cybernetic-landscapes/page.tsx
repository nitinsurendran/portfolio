...
type ManifestBlock =
  ...
  | { type: "divider" }
  | { type: "imageLarge"; media: { kind: "image" | "video"; src: string; poster?: string }; width?: number; adaptive?: boolean }
  | {
      type: "imageSmall";
  ...
  | { type: "impact"; title?: string; content?: string }
  | { type: "otherProjects" }
...
function renderBlock(
  ...
  switch (block.type) {
    ...
    case "imageLarge":
      return <ImageLargeBlock key={index} media={block.media} width={block.width} adaptive={block.adaptive} />;
    case "imageSmall":
    ...
    case "impact":
      return <Impact key={index} title={block.title} content={block.content} />;
    case "otherProjects":
    ...
    default:
      if (process.env.NODE_ENV === "development") {
        console.warn("[CyberneticLandscapesPage] Unknown block type:", (block as { type: string }).type);
      }
      return null;
  }
}

export default function CyberneticLandscapesPage() {
  const [manifest, setManifest] = useState<Manifest | null>(null);
  const [error, setError] = useState<string | null>(null);
  const rootRef = useRef<HTMLDivElement>(null);

  const project = getProjectBySlug("cybernetic-landscapes");
  const projectBadges = project?.badges;
  const projectTitle = project?.title;
  const projectDescription = project?.description;

  useEffect(() => {
    fetch("/media/projects/cybernetic-landscapes/manifest.json", { cache: "no-store" })
      .then((res) => {
        if (!res.ok) {
