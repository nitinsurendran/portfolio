...
import { digitalRamsOverview, digitalRamsCopy, type DigitalRamsContentKey } from "@/content/projects/digital-rams";
...
type ManifestBlock =
  ...
  | { type: "text"; heading?: string; content?: string; contentKey?: DigitalRamsContentKey }
...
function renderBlock(
  ...
  switch (block.type) {
    case "header": {
      ...
      return (
        <HeaderBlock
          ...
          overviewRight={digitalRamsOverview}
    ...
    case "text": {
      ...
      let content: string;
      if (block.contentKey) {
        const section = digitalRamsCopy[block.contentKey];
    ...
    default:
      if (process.env.NODE_ENV === "development") {
        console.warn("[DigitalRamsPage] Unknown block type:", (block as ManifestBlock & { type: string }).type);
      }
      return null;
  }
}

export default function DigitalRamsPage() {
  const [manifest, setManifest] = useState<Manifest | null>(null);
  const [error, setError] = useState<string | null>(null);
  const rootRef = useRef<HTMLDivElement>(null);

  const project = getProjectBySlug("digital-rams");
  const projectBadges = project?.badges;
  const projectTitle = project?.title;
  const projectDescription = project?.description;

  useEffect(() => {
    fetch("/media/projects/digital-rams/manifest.json", { cache: "no-store" })
      .then((res) => {
        if (!res.ok) {
  ...
  return (
    <ProjectMediaBaseProvider basePath="/media/projects/digital-rams">
      <main className="pt-[144px] pb-0 relative">
        <Container>
          <div ref={rootRef}>
